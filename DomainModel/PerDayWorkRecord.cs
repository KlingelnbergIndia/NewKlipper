    ...
    public class PerDayWorkRecord
    {
        ...
        public TimeSpan GetTimeIn()
        {
            return CalculateAbsoluteOutTimeAndInTime(
                ...
                .Select(x => x.EventTime.TimeOfDay).Min(), 
                   AbsoluteTime.TimeIn);
        }

        public TimeSpan GetTimeOut()
        {
            var AccessEventsOFServerRoom = _accessEvents
                .Where(x =>x.FromServerRoom());

            var AccessEventsOfUSBRoom = _accessEvents
                .Where(x => x.FromUSBDeviceDoor());

            var AccessEventForTimeOut = _accessEvents
                .Except(AccessEventsOFServerRoom)
                .ToList();

            AccessEventForTimeOut= AccessEventForTimeOut
                .Except(AccessEventsOfUSBRoom)
                .ToList();

            var minTime = AccessEventForTimeOut
                .Select(x => x.EventTime.TimeOfDay)
                .Min();

            var maxTime = AccessEventForTimeOut
                .Select(x => x.EventTime.TimeOfDay)
                .Max();
        ...
        public TimeSpan CalculateWorkingHours()
        {
            ...
            var GymnasiumPointAccessEvents = 
                new WorkLogs(accessEventsOfGymEntry);

            var MainEntryPointAccessEvents = 
                new WorkLogs(accessEventsOfMainEntry);

            return GetWorkingHours(accessEventsOfMainEntry,
                accessEventsOfGymEntry,timeIn, timeOut,
                GymnasiumPointAccessEvents,
                MainEntryPointAccessEvents);
        }

        private static TimeSpan GetWorkingHours(
            List<AccessEvent> accessEventsOfMainEntry,
            List<AccessEvent> accessEventsOfGymEntry, 
            TimeSpan timeIn, TimeSpan timeOut,
            WorkLogs GymnasiumPointAccessEvents,
            WorkLogs MainEntryPointAccessEvents)
        {
